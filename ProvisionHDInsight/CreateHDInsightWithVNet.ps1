...
$VNetName = "**************"
$SubnetName = "**************"
$VNetId = (Get-AzureVNetSite -VNetName $VNetName).Id
...
# Create cluster 
$config = New-AzureHDInsightClusterConfig -ClusterType Hadoop -ClusterSizeInNodes $DataNodesNumber -HeadNodeVMSize $HeadNodeVMSize -DataNodeVMSize $DataNodeVMSize -VirtualNetworkId $VNetId -SubnetName $SubnetName |
...
New-AzureHDInsightCluster -Config $config -Credential $ClusterCreds -Name $ClusterName -Location $ClusterLocation -Version $ClusterVersion -EndPoint $Endpoint
