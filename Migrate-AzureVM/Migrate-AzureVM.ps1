<#
.Description
Move Azure IaaS VM vhd files from one storage to another storage account and keep vm configuration unchanged

.Example
    .\Move-AzureVHD.ps1 -CloudServiceName "blairvhdmove" -VMName "blairvhdmove" -DestStorageAccountName "blairstorage" -VNetName "RVNET-SH01"

.Program Manager : Qing Liu qliu@microsoft.com
.Dev: Blair Chen blairch@microsoft.com 
.Code Reviewer : Qing Liu qliu@microsoft.com; Wei Sun, weisun@microsoft.com 
#>
...
#######################################################################
$sourceVM = Get-AzureVM ¨CServiceName $CloudServiceName ¨CName $VMName
if ( $sourceVM -eq $null )
{
...
if ( $sourceVM.Status -notmatch "Stopped" )
{
    ...
    $VMStatus = (Get-AzureVM ¨CServiceName $CloudServiceName ¨CName $vmName).Status
...
$sourceContext = New-AzureStorageContext ¨CStorageAccountName $sourceStorageAccountName -StorageAccountKey $sourceStorageKey -Environment $cloudEnv
...
$destContext = New-AzureStorageContext ¨CStorageAccountName $DestStorageAccountName -StorageAccountKey $destStorageKey -Environment $cloudEnv
...
Remove-AzureVM ¨CServiceName $CloudServiceName ¨CName $VMName
...
do
{
    ...
    Sleep -Seconds 10
    $ip = Test-AzureStaticVNetIP ¨CVNetName $VNetName ¨CIPAddress $sourceVM.IpAddress
